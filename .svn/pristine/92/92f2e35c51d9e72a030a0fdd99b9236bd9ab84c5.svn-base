...
$(document).ready(function(){
	...
	var rankReviewTarget = $('#ranking-review')
	...
	$.ajax({
		...
		success:function(result){
			...
			var rankingReviewList = JSON.parse(result.rankingReviewList);
			...
			eventLoopReview(rankReviewTarget, rankingReviewList);
		}
	});
...
function getDate(date){
	...
	var day = date.split(' ')[1].substr(0,1);
	
	var setDate = year+'년 '+month+' '+day+'일';
...
function eventLoopReview(target, list){
	
	for(var i = 0; i < list.length; i++){

        var no = list[i].no;
        var imagePath = 'image/REVIEW/'+list[i].imagePath;

        var deleted = list[i].deleted;
        var reviewId = 'review-'+no;

        var innerhtml = '<div class="col-3 text-center">'
        				+'<a onclick="openPopUp2('+no+')" href="#" data-toggle="modal" data-target="#review-column-01">'
        				+'<img src="'+imagePath+'" alt="" class="img-thumbnail" style="height:270px;"/>'
        				+'</a>'
        				+'</div>'
...
function openPopUp2(no){
	...
	$.ajax({
		...
		success:function(data){
         ...
         document.querySelector("#del-btn").setAttribute("href" , "review/deletereview.jsp?productNo="+review.ProductNo+"&reviewNo=" + no)
