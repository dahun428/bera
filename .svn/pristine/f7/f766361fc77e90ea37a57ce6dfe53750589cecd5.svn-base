...
$(document).ready(function(){
	...
	$('#custom-nav').bind({
		mouseenter: function() {
			var $btn=$('.nav-sub');
			if(!$btn.is(':animated')) $btn.slideDown(500);
		},
		mouseleave: function() {
			$('.nav-sub').slideUp(500);
		}
...
//event function
	...
	$('#event-form-btn').click(function(){
		...
		if($('#event-manager-event').is(':checked')){
			...
			if($('#change-image-thumbnail').attr('src') == '../image/no_detail_img.jpg'){
				alert('사진을 등록해주세요');
				return;
			}
			...
			if($('#change-image-content').attr('src') == '../image/no_detail_img.jpg'){
				alert('사진을 등록해주세요');
				return;
			}
		} else {
			...
			if($('#change-image-banner').attr('src') == '../image/no_detail_img.jpg'){
				alert('사진을 등록해주세요');
				return;
			}
		}
