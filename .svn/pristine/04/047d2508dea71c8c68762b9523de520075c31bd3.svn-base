...
public class EventService {
	...
	public List<EventDto> getBannerEventAll() {
		return eventDao.getBannerEventAll();
	}
	public int getBannerEventAllCount() {
		return eventDao.getBannerEventAllCount();
	}
	//진행중인 이벤트 리스트
	...
	public void deleteEventByNo(int eventNo) throws SQLException{
		eventDao.deleteEventByNo(eventNo);
	}
	...
	public void updateEvent(EventDto event) throws SQLException{
		...
		if(event.getContentImagePath() != null) {
			...
			eventDao.updateEventImage(event.getNo(), event.getImagePath(), event.getEventType());
		}
		if(event.getThumbnailImagePath() != null) {
