					...
					<div class="col-12">
						<div class="row mt-4">
							...
							<div class="col-9">
								<!-- manager navi -->
								<%
									String managerPosition = "salesmanager";
								%>
									...
									<div class="row">
										<div class="col-4">
											<input type="hidden" id="type" value="<%=type%>"> <input
												type="hidden" id="month" value="<%=month%>"> <a
												class="btn btn-outline-dark" href="admin.jsp?type=total">Total</a>
											...
											<div class="col-12">
												<div class="jumbotron bg-dark text-white mb-1">
													<div class="row text-center">
														...
														<div class="col-6">
															<p class="display-4 text-center font-weight-bold"><%=NumberUtil.numberWithComma(price)%>원
															</p>
														</div>
														<%
														...
														<div class="col-6">
															<p class="display-4 text-center font-weight-bold"><%=NumberUtil.numberWithComma(mprice)%>원
															</p>
														</div>
														<%
														...
														<div class="col-6">
															<p class="display-4 text-center font-weight-bold"><%=NumberUtil.numberWithComma(wprice)%>원
															</p>
														</div>
														<%
														...
														<div class="col-6">
															<p class="display-4 text-center font-weight-bold"><%=NumberUtil.numberWithComma(dprice)%>원
															</p>
														</div>
														<%
															}
				...
				function drawDashboard() {
					...
					if ("monthly" === type) {
						for (var i = 0; i <= month; i++) {
							dataRow = [ new Date('2020', i), 10, 10,
									parseInt(monthData[i].value) ];
							data.addRow(dataRow);
						}
					} else if ("weekly" === type) {
						for (var i = 0; i < 5; i++) {
							dataRow = [
									new Date('2020', '06', '12', '10', i + 1),
									10, 10, parseInt(weekData[i].value) ];
							data.addRow(dataRow);
						}
					} else if ("daily" === type) {
						for (var i = 0; i < 31; i++) {
							dataRow = [ new Date('2020', month, i + 1), 10, 10,
									parseInt(dailyData[i].value) ];
							data.addRow(dataRow);
						}
					} else if ("total" === type) {
						for (var i = 0; i < totalData.length; i++) {
							dataRow = [ new Date('2020'), 10, 10,
									parseInt(totalData[i].value) ];
							//	  console.log(to)
							data.addRow(dataRow);
						}
					}
