...
function buildProductList(products) {
	document.querySelectorAll('#product-list div').forEach(function(curr) {
		curr.remove()
	})
	products
...
function adminInit(event, category) {
	document.querySelector('#add-product-btn').onclick = function(){
        location.href='/semi-project/product/productform.jsp'
    }
	buildList(event, category)
}

function buildList(event, category) {
	console.log('called build : ' + category)
	var url = "../product/getListDataForAdmin.jsp?category=" + category
	var list = document.querySelector('#product-list')
	var baseRow = document.querySelector('#product-row')
	var baseHeader = document.querySelector('#list-header')
	console.log(list)
	...
	xhr.onreadystatechange = function() {
		if (xhr.readyState == 4 && xhr.status == 200) {
			...
			var listHeader = baseHeader.cloneNode(true)
			list.appendChild(listHeader)
			listHeader.style.display = 'block'
			var text = xhr.responseText
			var productList = JSON.parse(text)
			productList.forEach(function(product) {
				var newRow = baseRow.cloneNode(true)
				newRow.setAttribute('id', 'product-row-' + product.no)
				newRow.style.display = 'block'
				list.appendChild(newRow)

				var newId = document.querySelector('#product-row-' + product.no
						+ ' #product-id')

				newId.setAttribute('id', 'product-id-' + product.no)
				newId.textContent = product.no
				var newCat = document.querySelector('#product-row-'
						+ product.no + ' #product-category')
				newCat.setAttribute('id', 'product-category-' + product.no)
				newCat.textContent = product.category
				var newTitle = document.querySelector('#product-row-'
						+ product.no + ' #product-name')
				newTitle.setAttribute('id', 'product-name-' + product.no)
				newTitle.textContent = product.name

				var newDelBtn = document.querySelector('#product-row-'
						+ product.no + ' #product-info')
				newDelBtn.setAttribute('id', 'product-delete-' + product.no)
				newDelBtn.onclick = openPop.bind(event, product.no)
			})
		}
	}
